...
const ContactForm = React.forwardRef<HTMLFormElement, { customSubmit?: boolean }>(
  ({ customSubmit = false }, imperativeRef) => {
    const [state, handleSubmit] = useForm(process.env.NEXT_FORMSPREE_CONTACTUS || 'xyzdopal');
    ...
    return (
      <form ref={ref} onSubmit={handleSubmit} className={styles.form__body}>
        ...
        {!customSubmit && (
          <div className="fbox">
            <Button type="submit" text={t('submit')} disabled={state.submitting} />
          </div>
        )}
        {state.succeeded && (
